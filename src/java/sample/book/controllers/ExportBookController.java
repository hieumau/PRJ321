 ...
 * @author saost
 */
public class ExportBookController extends HttpServlet {
...
//    private static final String ERROR = ViewLibraryController.class.getSimpleName();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = SUCCESS;
        try {
            int amount = Integer.parseInt(request.getParameter("export"));
            String bookID = request.getParameter("id");
            BookDAO bookDAO = new BookDAO();

            if (bookDAO.getAvailable(bookID) < amount){
                request.setAttribute("ERROR_MESSAGE", "Not enough book");
            } else {
                bookDAO.exportBook(Integer.parseInt(bookID), amount);
                request.setAttribute("SUCCESS_MESSAGE", "Export successful");
            }
        } catch (Exception e){
